...
class RegistrationAudit(Protocol):
    def save_request(self, request: RegistrationRequest) -> Any:
        ...
...
class RegistrationProcessor:
    """Coordinates approved, deterministic, idempotent registration submission."""
    ...
    def process(
        ...
        if request.status != RegistrationStatus.APPROVED:
            return ProcessingResult(
                ...
                processed=False,
                skipped=True,
                reason="registration_requires_approval",
            )
