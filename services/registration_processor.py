...
class RegistrationAudit(Protocol):
    def record_transition(self, request_id, opportunity_id: str, from_status: str | None,
                          to_status: str, actor: str | None = None,
...
class RegistrationProcessor:
    ...
    def __init__(self, store: ProcessedDealStore, gateway: RegistrationGateway,
                 audit: RegistrationAudit | None = None) -> None:
        self.store = store
        self.gateway = gateway
        self.audit = audit

    def _record(self, request: RegistrationRequest, previous: RegistrationStatus,
                reason: str | None = None) -> None:
        if self.audit:
            self.audit.record_transition(
                request.request_id, request.opportunity_id,
                previous.value, request.status.value, reason=reason,
            )
    ...
    def process(self, deal: Deal, request: RegistrationRequest, payload: dict[str, Any]) -> ProcessingResult:
        ...
        if self.store.has_processed(deal.opportunity_id, fingerprint):
            return ProcessingResult(request=request, processed=False, skipped=True,
        ...
        if not getattr(result, "accepted", False):
            ...
            request.transition(RegistrationStatus.SUBMISSION_FAILED,
                               reason=getattr(result, "message", "submission_failed"))
            self._record(request, previous, request.error)
            return ProcessingResult(request=request, processed=False, skipped=False,
